...
module.exports = (sequelize, modelName, tableName) => {
  // Aux function to execute SQL queries and handle errors
  ...
  // Aux function to validate model data
  const validateModel = async (modelData, modelOptions) => {
    const modelInstance = sequelize
      .model(modelName)
      .build(modelData, { isNewRecord: modelOptions.isNewRecord });
    try {
  ...
  return {
    create: async (req, res) => {
      // Initial data from request body
      ...
      // Validate using model's rules before inserting
      const validationError = await validateModel(userData, { isNewRecord: true });
      if (validationError) {
        return res.status(400).json({ errors: validationError.errors.map((e) => e.message) });
      }
    ...
    update: async (req, res) => {
      ...
      const validationError = await validateModel(updateData, { isNewRecord: false });
      if (validationError) {
        return res.status(400).json({ errors: validationError.errors.map((e) => e.message) });
      }
      ...
      for (const key of updateKeys) {
        // Skip null values, except for Schedule model
